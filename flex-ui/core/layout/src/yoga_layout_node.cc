...
// Bring node_props constants into scope (they live in flexui::core::vdom).
using namespace flexui::core::vdom;  // NOLINT(build/namespaces)
...
#define YG_SET_NUMBER_PERCENT_AUTO_DECL(NAME)                                                      \
  void YogaLayoutNode::SetYG##NAME(std::shared_ptr<flexui::common::FlexUIValue> hippy_value) {    \
    flexui::common::FlexUIValue::Type type = hippy_value->GetType();                              \
    if (type == flexui::common::FlexUIValue::Type::kNumber) {                                     \
      auto value = static_cast<float>(hippy_value->ToDoubleChecked());                             \
      YGNodeStyleSet##NAME(yoga_node_, value);                                                     \
    } else if (type == flexui::common::FlexUIValue::Type::kString) {                              \
...
#define YG_SET_EDGE_NUMBER_PRECENT_DECL(NAME)                                                                \
  void YogaLayoutNode::SetYG##NAME(YGEdge edge, std::shared_ptr<flexui::common::FlexUIValue> hippy_value) { \
    flexui::common::FlexUIValue::Type type = hippy_value->GetType();                                        \
    if (type == flexui::common::FlexUIValue::Type::kNumber) {                                               \
      auto value = static_cast<float>(hippy_value->ToDoubleChecked());                                       \
...
static YGEdge GetYGEdgeFromEdge(flexui::core::layout::Edge edge) {
  if (flexui::core::layout::Edge::EdgeLeft == edge) {
    return YGEdge::YGEdgeLeft;
  } else if (flexui::core::layout::Edge::EdgeTop == edge) {
    return YGEdge::YGEdgeTop;
  } else if (flexui::core::layout::Edge::EdgeRight == edge) {
    return YGEdge::YGEdgeRight;
  } else if (flexui::core::layout::Edge::EdgeBottom == edge) {
...
void YogaLayoutNode::CalculateLayout(float parent_width, float parent_height, Direction direction,
  ...
  } else if (direction == flexui::core::layout::Direction::LTR) {
    yoga_direction = YGDirectionLTR;
  } else if (direction == flexui::core::layout::Direction::RTL) {
...
void YogaLayoutNode::ResetLayoutCache() {
  // FlexUI: no-op in Yoga; cache invalidated via MarkDirty.
}
...
static YGSize YGMeasureFunction(YGNodeConstRef node, float width, YGMeasureMode width_mode, float height,
                                YGMeasureMode height_mode) {
  auto yoga_node = reinterpret_cast<YogaLayoutNode*>(YGNodeGetContext(const_cast<YGNodeRef>(node)));
...
void YogaLayoutNode::SetMeasureFunction(MeasureFunction measure_function) {
  ...
  YGNodeSetMeasureFunc(yoga_node_, YGMeasureFunction);
}
