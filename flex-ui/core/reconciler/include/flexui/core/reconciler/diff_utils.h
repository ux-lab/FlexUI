 ...
 * Absorbed from Hippy dom/diff_utils.h into FlexUI core/reconciler.
 * Changes: namespace hippy::dom -> flexui::core::reconciler;
...
using DomValueMap = typename std::unordered_map<std::string, std::shared_ptr<FlexUIValue>>;
using DomValueObject = typename std::unordered_map<std::string, FlexUIValue>;
using DomValueArray = typename std::vector<FlexUIValue>;
