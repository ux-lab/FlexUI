 ...
 * modules/footstone/include/footstone/worker_impl.h in the Hippy project.
 *
...
class WorkerImpl: public Worker, public std::enable_shared_from_this<WorkerImpl> {
 public:
  WorkerImpl(std::string name = "", bool is_schedulable = true,
             std::unique_ptr<Driver> driver = std::make_unique<CVDriver>()):
