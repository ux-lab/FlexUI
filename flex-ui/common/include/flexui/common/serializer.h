...
class Serializer {
 public:
  ...
  void WriteValue(const FlexUIValue& value);

  /**
   * @brief Get serialized data for FlexUIValue. After Release, the pointer
   * is owned by the caller and must be freed with free().
   */
 ...
 private:
  ...
  void WriteDenseJSArray(const FlexUIValue::FlexUIValueArrayType& array);

  void WriteJSObject(const FlexUIValue::FlexUIValueObjectType& obj);
 ...
 private:
  ...
  void WriteObject(const FlexUIValue& value);
