 ...
 * modules/footstone/src/worker.cc in the Hippy project.
 *
 ...
 *   - Removed platform-specific looper_driver includes (Android/iOS).
...
#include "flexui/common/cv_driver.h"
...
Worker::~Worker() {
  FLEXUI_CHECK(driver_->IsTerminated()) << "Terminate function must be called before destruction";
...
std::shared_ptr<TaskRunner> Worker::GetCurrentTaskRunner() {
  FLEXUI_CHECK(is_task_running) << "GetCurrentTaskRunner cannot be run outside of the task ";
  return local_runner;
}
...
std::unique_ptr<Task> Worker::GetNextTask() {
  ...
  for (auto &running_group : running_group_list_) {
    auto runner = running_group.back();
    auto task = runner->GetNext();
    if (task) {
      curr_group = running_group;
      local_runner = runner;
      return task;
    } else {
      if (!idle_task) {
...
void Worker::UpdateSpecificKeys(uint32_t task_runner_id,
  ...
  worker_key_map_[task_runner_id] = std::move(array);
}
...
void Worker::UpdateSpecific(uint32_t task_runner_id,
  ...
  specific_map_[task_runner_id] = array;
}
