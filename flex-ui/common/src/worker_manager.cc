 ...
 * modules/footstone/src/worker_manager.cc in the Hippy project.
 *
 * The FlexUI modifications:
 *   - Moved namespace `footstone` -> `flexui::common`.
 *   - Renamed include path `footstone/...` -> `flexui/common/...`.
 *   - Renamed macros `FOOTSTONE_CHECK` -> `FLEXUI_CHECK`,
 *     `FOOTSTONE_DCHECK` -> `FLEXUI_DCHECK`.
 *   - WorkerImpl name changed from "Hippy URI Worker" -> "FlexUI Worker".
...
void WorkerManager::CreateWorkers(uint32_t size) {
  ...
  for (uint32_t i = 0; i < size; ++i) {
    worker = std::make_shared<WorkerImpl>("FlexUI Worker");
    worker->Start();
...
void WorkerManager::Balance(int32_t increase_worker_count) {
  if (increase_worker_count > 0) {
    ...
    std::vector<uint32_t> cur_worker_group(static_cast<size_t>(size));
    ...
    for (auto i = index_; i < index_ + size; ++i) {
      ...
      auto cnt = resize_group_size - cur_worker_group[static_cast<size_t>(index)];
    ...
    auto size = static_cast<int32_t>(size_) + increase_worker_count;
    FLEXUI_DCHECK(size > 0);
    if (size <= 0) {
      return;
    }
    std::list<std::vector<std::shared_ptr<TaskRunner>>> groups;
    int32_t reduce_size = 0 - increase_worker_count;
    ...
    FLEXUI_DCHECK(!reduce_size) << "resize failed, The thread has too many unschedulable runners";
    if (reduce_size > 0) {
      return;
    }
    ...
    FLEXUI_DCHECK(size_ > 0);
    if (size_ <= 0) {
      return;
    }
    std::list<std::vector<std::shared_ptr<TaskRunner>>> groups;
    std::vector<uint32_t> cur_worker_group(size_);
    uint32_t groups_size = 0;
    for (uint32_t i = 0; i < size_; ++i) {
      groups_size += workers_[i]->GetRunningGroupSize();
      groups.splice(groups.end(), workers_[i]->RetainActiveAndUnschedulable());
    ...
    auto resize_group_size = static_cast<uint32_t>(std::ceil(groups_size / size_));
    auto groups_it = groups.begin();
    for (uint32_t i = 0; i < size_; ++i) {
      auto worker = workers_[i];
      auto cnt = static_cast<int32_t>(resize_group_size - cur_worker_group[i]);
      if (cnt <= 0) {
        continue;
      }
      std::list<std::vector<std::shared_ptr<TaskRunner>>> list;
