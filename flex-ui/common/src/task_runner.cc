...
TaskRunner::TaskRunner(std::string name)
    : TaskRunner(kDefaultGroupId, kDefaultPriority, true, std::move(name)) {}
...
int32_t TaskRunner::RunnerKeyCreate(const std::function<void(void*)>& destruct) {
  FLEXUI_CHECK(Worker::IsTaskRunning()) << "RunnerKeyCreate cannot be run outside of the task";
...
bool TaskRunner::RunnerKeyDelete(int32_t key) {
  FLEXUI_CHECK(Worker::IsTaskRunning()) << "RunnerKeyDelete cannot be run outside of the task";
  auto task_runner_id = Worker::GetCurrentTaskRunner()->GetId();
  std::shared_ptr<Worker> worker = worker_.lock();
  FLEXUI_CHECK(worker);
...
bool TaskRunner::RunnerSetSpecific(int32_t key, void* p) {
  FLEXUI_CHECK(Worker::IsTaskRunning()) << "RunnerSetSpecific cannot be run outside of the task";
  auto task_runner_id = Worker::GetCurrentTaskRunner()->GetId();
  std::shared_ptr<Worker> worker = worker_.lock();
  FLEXUI_CHECK(worker);
...
void* TaskRunner::RunnerGetSpecific(int32_t key) {
  FLEXUI_CHECK(Worker::IsTaskRunning()) << "RunnerGetSpecific cannot be run outside of the task";
  auto task_runner_id = Worker::GetCurrentTaskRunner()->GetId();
  std::shared_ptr<Worker> worker = worker_.lock();
  FLEXUI_CHECK(worker);
...
void TaskRunner::RunnerDestroySpecifics() {
  FLEXUI_CHECK(Worker::IsTaskRunning()) << "RunnerDestroySpecifics cannot be run outside of the task";
  auto task_runner_id = Worker::GetCurrentTaskRunner()->GetId();
  std::shared_ptr<Worker> worker = worker_.lock();
  FLEXUI_CHECK(worker);
