...
void Serializer::WriteValue(const FlexUIValue& value) {
  WriteObject(value);
}
...
void Serializer::WriteDenseJSArray(const FlexUIValue::FlexUIValueArrayType& array) {
  uint32_t length = flexui::common::check::checked_numeric_cast<size_t, uint32_t>(array.size());
...
void Serializer::WriteJSObject(const FlexUIValue::FlexUIValueObjectType& obj) {
  uint32_t length = flexui::common::check::checked_numeric_cast<size_t, uint32_t>(obj.size());
...
void Serializer::WriteObject(const FlexUIValue& value) {
  ...
  switch (type) {
    ...
    case FlexUIValue::Type::kNumber: {
      FlexUIValue::NumberType number_type = value.GetNumberType();
      switch (number_type) {
        case FlexUIValue::NumberType::kInt32: {
          WriteInt32(value.ToInt32Checked());
          break;
        }
        case FlexUIValue::NumberType::kUInt32: {
          WriteUint32(value.ToUint32Checked());
          break;
        }
        case FlexUIValue::NumberType::kDouble: {
          WriteDouble(value.ToDoubleChecked());
          break;
        }
        default: {
          FLEXUI_UNREACHABLE();
    ...
    case FlexUIValue::Type::kString: {
      WriteString(value.ToStringChecked());
      break;
    }
    case FlexUIValue::Type::kObject: {
      WriteJSObject(value.ToObjectChecked());
      break;
    }
    case FlexUIValue::Type::kArray: {
      WriteDenseJSArray(value.ToArrayChecked());
      break;
    }
    default: FLEXUI_UNREACHABLE();
  }
...
void Serializer::ExpandBuffer(size_t required_capacity) {
  ...
  FLEXUI_DCHECK(new_buffer != nullptr);
