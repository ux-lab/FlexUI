 ...
 * modules/footstone/src/base_timer.cc in the Hippy project.
 *
...
void BaseTimer::Reset() {
  // Note: scheduled_run_time_ <= TimePoint::Now() uses <= (not <) to work around
  // a precision bug observed on iPhone 12 Pro Max (iOS 16.2) where two rapid
  // TimePoint::Now() calls return identical nanosecond values.
