...
flexui::core::commit_pipeline::NodeHandle CreateNode() {
  ArkUI_NodeHandle h = NodeApi()->createNode(ARKUI_NODE_SCROLL);
  FLEXUI_TLOG(Component, ScrollViewNativeCreate, DEBUG) << "handle=" << h;
  return h;
}
void ApplyProps(flexui::core::commit_pipeline::NodeHandle, const ScrollViewProps&) {
  FLEXUI_TLOG(Component, ScrollViewApplyProps, DEBUG);
}
