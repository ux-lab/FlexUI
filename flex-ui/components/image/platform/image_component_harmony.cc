...
flexui::core::commit_pipeline::NodeHandle CreateNode() {
  ArkUI_NodeHandle h = NodeApi()->createNode(ARKUI_NODE_IMAGE);
  FLEXUI_TLOG(Component, ImageNativeCreate, DEBUG) << "handle=" << h;
  return h;
}
void ApplyProps(flexui::core::commit_pipeline::NodeHandle, const ImageProps&) {
  FLEXUI_TLOG(Component, ImageApplyProps, DEBUG);
}
